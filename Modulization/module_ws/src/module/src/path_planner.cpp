...
PathPlanner::PathPlanner(double _radius, double _x, double _y, double _z, double _dist, PathPlannerConfig& _pathPlannerConfig):m_pathPlannerConfig(_pathPlannerConfig){
	...
	m_theta_shift = m_pathPlannerConfig.theta_shift;
	m_theta_move = m_pathPlannerConfig.theta_move;
	m_phi_shift = m_pathPlannerConfig.phi_shift;
	m_phi_move = m_pathPlannerConfig.phi_move;
	m_num_angle_theta = m_pathPlannerConfig.num_angle_theta;
	m_num_angle_phi = m_pathPlannerConfig.num_angle_phi;
...
void PathPlanner::genPathNPose(){

	
	double theta_resol = m_theta_move/m_num_angle_theta;
	double phi_resol = m_phi_move/m_num_angle_phi;
	
	m_pathsize = (m_num_angle_theta+1)*(m_num_angle_phi+1);
	...
	for (int j = 0; j <= m_num_angle_theta; ++j) {
		double theta_rad = (m_theta_move/m_num_angle_theta * j + m_theta_shift) * M_PI/180.0;

		if (j % 2 == 0) {
			// even layer: phi + dir
			for (int i = 0; i <= m_num_angle_phi; ++i) {
				double phi_rad = (m_phi_move/m_num_angle_phi * i + m_phi_shift) * M_PI/180.0;
			...
			// odd layer: phi - dir
			for (int i = m_num_angle_phi; i >= 0; --i) {
				double phi_rad = (m_phi_move/m_num_angle_phi * i + m_phi_shift) * M_PI/180.0;
...
void PathPlanner::calcTempRotMat(double _phi, double _theta){
...
void PathPlanner::calcRobotQuaternion(Eigen::Matrix3d _mat, int _i)
{
...
std::vector<geometry_msgs::Pose> PathPlanner::makeWaypoints() {
